					...
					while (true) {
						...
						try {
							...
							for (int i = 0; i < gold.size(); i++) {
								...
								else if (gold.get(i).getDropHeight() == 0
										&& gold.get(i).canMove()) {
									Thread.sleep(500);
								}
								gold.get(i).move();
							...
							for (int j = goldRemoval.size() - 1; j >= 0; j--) {
								gold.remove(goldRemoval.get(j));
								world.setGoldBagList(gold);
								goldRemoval.remove(j);
							}
...
//		endScreen.add(scoreLabel, BorderLayout.SOUTH);
